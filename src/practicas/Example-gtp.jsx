const Filters = ({ filtersSelected, setFiltersSelected, getData }) => {
  ...
  const onChangeSelect = (option, name) => {
    ...
    if (name === 'position') {
      ...
      setFiltersSelected((prev) => ({ ...prev, arrDistricts: dataDistricts || [] }));
    }

    if (name === 'district') {
      let dataProvincialSections = yearJSON
        ?.find((e) => e.IdEleccion === filtersSelected.election)
        ?.Cargos.find((e) => Number(e.IdCargo) === Number(filtersSelected.position))
        ?.Distritos.find((e) => e.IdDistrito === value).SeccionesProvinciales;

      if (dataProvincialSections && !dataProvincialSections[0].IDSeccionProvincial) {
        dataProvincialSections = dataProvincialSections[0].Secciones.map((section) => ({
          value: section.IdSeccion,
          label: section.Seccion,
        }));

        // validación porque para presidente no hay secciones
        if (dataProvincialSections[0].value) {
          setFiltersSelected((prev) => ({
            ...prev,
            provincialSection: '',
            section: '',
            arrProvincialSections: [],
            arrSections: dataProvincialSections || [],
          }));
        } else {
          setFiltersSelected((prev) => ({
            ...prev,
        ...
        setFiltersSelected((prev) => ({
          ...
          provincialSection: '',
          section: '',
          arrProvincialSections: dataProvincialSections.map((province) => ({
  ...
  const getSelects = () =>
    selectValues.map(({ name, options, label }) => {
      if (name !== 'provincialSection' || filtersSelected.arrProvincialSections.length > 0) {
        return (
          <Select
            name={name}
